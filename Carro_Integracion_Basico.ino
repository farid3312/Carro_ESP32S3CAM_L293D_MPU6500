...
// ===== PINES L293D =====
const int motorPin1 = 39; // IN1 - Motor A
const int motorPin2 = 42; // IN2 - Motor A
const int enAPin    = 40; // ENA - Motor A

const int motorPin3 = 14; // IN3 - Motor B
const int motorPin4 = 41; // IN4 - Motor B
const int enBPin    = 38; // ENB - Motor B
...
#define CAM_FRAME_INTERVAL_MS 33
...
void carCmd(char dir) {
  switch (dir) {
    ...
    case 'B': // Atras
      ...
      ledcWrite(enBPin, motorSpeed);
      break;
    case 'L': // Giro Izquierda: Motor A atras, Motor B adelante
      digitalWrite(motorPin1, HIGH); digitalWrite(motorPin2, LOW);
      digitalWrite(motorPin3, HIGH); digitalWrite(motorPin4, LOW);
      ledcWrite(enAPin, motorSpeed);
      ledcWrite(enBPin, motorSpeed);
      break;
    case 'R': // Giro Derecha: Motor A adelante, Motor B atras
      digitalWrite(motorPin1, LOW);  digitalWrite(motorPin2, HIGH);
      digitalWrite(motorPin3, LOW);  digitalWrite(motorPin4, HIGH);
      ledcWrite(enAPin, motorSpeed);
      ledcWrite(enBPin, motorSpeed);
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #fps-display { position: absolute; top: 10px; left: 10px; background: rgba(0,0,0,.7); color: #0f0; padding: 4px 9px; font-weight: bold; border-radius: 4px; font-size: 15px; pointer-events: none; }
    #ws-status { position: absolute; top: 10px; right: 10px; background: rgba(0,0,0,.7); color: #f90; padding: 4px 9px; font-weight: bold; border-radius: 4px; font-size: 12px; pointer-events: none; }
...
<script>
  function carCmd(dir) { fetch('/car?d=' + dir).catch(e => console.error(e)); }
  function updateSpeed(v) { document.getElementById('spd-val').innerText = Math.round(v / 255 * 100) + '%'; }
  function sendSpeed(v) { updateSpeed(v); fetch('/speed?v=' + v).catch(e => console.error(e)); }
  ...
  function initWebSocket() {
    ...
    websocket.onopen = function() { reconnectDelay = 1000; st.innerText = 'WS: Conectado'; st.style.color = '#0f0'; };
    websocket.onclose = function() {
      st.innerText = 'WS: Desconectado'; st.style.color = '#f00';
      if (reconnectTimer) clearTimeout(reconnectTimer);
      reconnectTimer = setTimeout(function() { reconnectDelay = Math.min(reconnectDelay * 2, 16000); initWebSocket(); }, reconnectDelay);
    };
    ...
    websocket.onmessage = function(event) {
      if (typeof event.data === 'string') {
        ...
        frameCount++;
        var now = Date.now();
        if (now - lastTime >= 1000) { document.getElementById('fps-display').innerText = 'FPS: ' + frameCount; frameCount = 0; lastTime = now; }
      }
    };
  }
...
void setup() {
  ...
  WiFi.setTxPower(WIFI_POWER_20dBm); WiFi.setAutoReconnect(true); WiFi.persistent(false);
  WiFi.begin(ssid, password);
  Serial.print("WiFi");
  int t = 0;
  ...
  if (client) {
    if (now - tiempoAnteriorGY >= 100) {
      ...
      if (Wire.available() == 14) {
        ...
        if (!client->queueIsFull()) client->text(info);
      }
    }
